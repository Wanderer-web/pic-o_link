...
{
    systick_delay_ms(550);  //发送进入AT模式的命令前需要保证模块在550ms内没有接收过任何数据
    at_mode = 1;            //进入AT模式
...
void bluetooth_ch9141_exit_at_mode(void)
{
    ...
    at_mode = 0;            //进入透传模式
    systick_delay_ms(300);  //等待成功进入AT模式
}
...
CH9141_STATUS_enum bluetooth_ch9141_get_status(CH9141_MODE_enum mode)
{
    ...
    ch9141_status = (CH9141_STATUS_enum)((at_mode_data[0] - '0') * 10 + (at_mode_data[1] - '0'));
    if(SLAVE_MODE == mode)
    {
...
void bluetooth_ch9141_init(CH9141_MODE_enum mode, int8 *salve_mac_password)
{
    ...
    //初始化流控引脚
    gpio_init(BLUETOOTH_CH9141_RTS_PIN, GPI, 0, GPIO_PIN_CONFIG);
    ...
    uart_set_handle(BLUETOOTH_CH9141_UART, &bluetooth_ch9141_g_lpuartHandle, bluetooth_ch9141_uart_callback, NULL, 0, bluetooth_ch9141_receivexfer.data, 1);
    
    //开启串口中断
    uart_rx_irq(BLUETOOTH_CH9141_UART, 1);
    //NVIC_SetPriority((IRQn_Type)((IRQn_Type)(BLUETOOTH_CH9141_UART) + LPUART1_IRQn),0);  //设置中断优先级 范围0-15 越小优先级越高
    
    
    irq_status = DisableGlobalIRQ();
    EnableGlobalIRQ(0);
